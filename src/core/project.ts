...
export function findProjectRoot(startDir: string): string {
  ...
  while (true) {
    ...
    if (parent === dir) {
      throw new Error(
        `Not inside a Node.js project (no package.json found). Run \`${CLI_NAME} init\` first.`,
      );
    }
    dir = parent;
  }
...
export function createProjectContext(startDir: string): ProjectContext {
  ...
  if (!existsSync(appEntryPath)) {
    throw new Error(
      `App entry not found at ${config.appEntry}. Run \`${CLI_NAME} init\` or restore ${DEFAULT_APP_ENTRY}.`,
    );
  }
  const compositionPath = join(root, config.srcDir, APP_DIR, COMPOSITION_BASENAME);
  if (!existsSync(compositionPath)) {
    throw new Error(
      `Composition entry not found at ${config.srcDir}/${APP_DIR}/${COMPOSITION_BASENAME}. Run \`${CLI_NAME} init\` or restore it.`,
    );
  }
